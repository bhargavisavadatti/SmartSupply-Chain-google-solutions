...
if (firebaseConfig.apiKey === "YOUR_FIREBASE_API_KEY") {
  console.log("Using Mock DB fallback mode");
  ...
  db = {
    collection: function(c) {
      return {
        ...
        get: async function() {
          if (c === 'shipments') {
            if (mockShipments.length === 0 && typeof SEED_SHIPMENTS !== 'undefined') {
              mockShipments = JSON.parse(JSON.stringify(SEED_SHIPMENTS));
            }
            let res = mockShipments;
            if (this._filterId) res = res.filter(s => s.id === this._filterId);
            return { empty: res.length === 0, docs: res.map(s => ({ 
               id: s.id, 
               data: () => s, 
               ref: { update: async (u) => { 
                   Object.assign(s, u); 
                   if (window._mockShipmentsSnapshotCb) {
                      window._mockShipmentsSnapshotCb({ docs: mockShipments.map(m => ({ id: m.id, data: () => m })) });
                   }
               } } 
            })) };
          }
          if (c === 'cashflow') {
            return { exists: true, data: () => DEFAULT_CASHFLOW };
          }
        ...
        onSnapshot: function(cb) {
          if (c === 'shipments') {
            ...
            cb({ docs: mockShipments.map(s => ({ id: s.id, data: () => s })) });
            window._mockShipmentsSnapshotCb = cb;
          } else {
            cb({ docs: [] });
          }
        }
    ...
    batch: () => ({ 
      set: () => {}, 
      update: (ref, data) => ref.update && ref.update(data), 
      commit: async () => {} 
    })
  };
} else {
